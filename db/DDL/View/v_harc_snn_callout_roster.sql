...
/*
* v_harc_snn_callout_roster
* -------------------------
*
* Author:  Tim Collier
* Date:  November 15, 2024
*
* Purpose:  Generate a list of callsigns to call during the Henderson Amateur Radio Club's Sunday Night Net.
*           Selection is based on last checkin date.  If the last checkin date was less than the cutoff number of
*           days, the callsign will be selected for that week's callout roster.
*           If one is known to the system, the person's preferred name will be used in place of theif first name.
*           There is also a list of callsigns that will be added to the roster regardless of when they last
*           checked in.  These are the net control operators.
*           A list of silent keys is also included so that their call signs will NOT appear on the roster.
*/
    SELECT DISTINCT a.call AS Callsign,
                    CASE WHEN v_pers.pers_call IS NULL THEN a.f_nam ELSE ifnull(v_pers.pers_pref_nam, v_pers.pers_f_nam) END AS [First Name],
...
* There may be some special cases where a call sign is always included */
   ...
   qso_date must be within the call_out_cutoff_days (v_current_call_out_cutoff_days)
...
AND
           ...
           , 'KK7HGW'/*Kenyon Spevak Net Control Operator */
